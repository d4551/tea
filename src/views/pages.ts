...
export const renderHomePage = (input: PageRenderInput): string => {
  ...
  const sceneLauncher = renderCollapse({
    ...
    className: "rounded-box border border-base-300/50",
  ...
  const heroContent = renderHero({
    ...
    subtitle: messages.pages.home.welcomeBack,
    actions: `${renderDrawerToggleControl({
      ...
      <a href="${withLocaleQuery(appRoutes.builder, locale)}" class="btn btn-primary gap-2" aria-label="${escapeHtml(messages.pages.home.openUnifiedBuilder)}">
  ...
  <div class="grid gap-4 p-4 md:p-6">
    ...
    ${renderStats({
      ...
      vertical: true,
      className: "w-full sm:w-auto bg-base-200",
      ariaLabel: messages.pages.home.statsScenes,
    })}
